...
对于高级版和 BYOL 许可证，磁盘限制可防止您单独使用磁盘来达到 368 TB 容量限制。您可以通过达到 368 TB 容量限制 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/concept-data-tiering.html["将非活动数据分层到对象存储"^]。有关详细信息，请参见以下磁盘限制。
...
| 将磁盘和冷数据分层到 Google Cloud 存储分段时的最大系统容量  a| 
* 高级： 368 TB
* BYOL ：每个许可证 368 TB
